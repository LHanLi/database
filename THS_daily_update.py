...
preday = today+datetime.timedelta(days=-30)
    ...
    # CFE data is not supported in THS
        if('CFE' not in j[0]):
            ...
            try:
                ...
                continue
            cursor.execute(insert)
            print('%s %s'%(date,normal(code)))
            
    db.commit()

# get trade days
THS_iFinDLogin('ifind6521','751676')
date = THS_Date_Query('212001','mode:1,dateType:0,period:D,dateFormat:0',str(preday),str(today))
date = date.data
date = date.split(',')
...
for day in date:
    isexist = "select 1 as isExist from trading_daily_price_1 where Date = '%s'"%(day)
    if(cursor.execute(isexist)):
        break;
    daily_commodity_update(day)
    print('success %s'%day)
